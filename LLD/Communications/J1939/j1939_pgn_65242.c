...
static void BuildPgn65242OnePacket (J1939_Transmit_Message_Info_T *TxMsg);
static can_boolean_t ServiceDm1TransportOut (TpOutStateType State,
                                             J1939_Transmit_Message_Info_T *TxMsg);
static can_boolean_t Service_PGN_65242 (J1939_Transmit_Message_Info_T *TxMsg);

/************************************************************************/
/* Global Variables                                                     */
/************************************************************************/
static uint8_t J1939_PGN_65242_Msg_Snapshot[J1939_SOFT_BUFFER_SIZE];
static uint16_t NumOfSoftBytes;

/***************************************************************************/
/* FUNCTION: BuildPgn65242OnePacket                                        */
/***************************************************************************/
static void BuildPgn65242OnePacket (J1939_Transmit_Message_Info_T *TxMsg)
{
   uint8_t NumPacked;
   ...
   TxMsg->ID = J1939_ADD_THIS_SRC_ADDR (J1939_PGN_65242_BASE_ID);
   TxMsg->Length = J1939_PGN_65242_LENGTH;

   NumPacked = 0;
   while (NumPacked < 8)
   {
      if (NumPacked >= NumOfSoftBytes)
      {
         TxMsg->Data[NumPacked] = 0xFF;
      }
      else
      {
         TxMsg->Data[NumPacked] = J1939_PGN_65242_Msg_Snapshot[NumPacked];
      }
...
/***************************************************************************/
static can_boolean_t ServicePGN65242TransportOut (TpOutStateType State,
                                             J1939_Transmit_Message_Info_T *TxMsg)
{
   ...
   else
   {
      ...
      while (NumPacked < 8)
      {
         if (DtcOffset < NumOfSoftBytes)
         {
            TxMsg->Data[NumPacked++] = J1939_PGN_65242_Msg_Snapshot[DtcOffset++];
         }
         else
         {
            TxMsg->Data[NumPacked++] = 0xFF;
...
static can_boolean_t Service_PGN_65242 (J1939_Transmit_Message_Info_T *TxMsg)
{
   ...
   TxMsg->Callback_Time_W = J1939_PGN_65242_FREQ;

   /* If previous DM1 still being transported then do not send another */
   if (J1939_CkPgnCurrentlyInTransportOut (J1939_PGN_65242))
   {
      /* If DM1 transmission already in progress then send Busy ACK if this
       * message was requested */
      if (TxMsg->Requested)
      {
         J1939_SendUnmanagedAcknowledgement (BusyAck, J1939_PGN_65242);
      }
   }
   else
   {
      NumOfSoftBytes = GetJ1939_SoftID_Size();
      J1939_Message_SnapShot ((uint8_t *)GetJ1939_SoftID_MsgPtr(), J1939_PGN_65242_Msg_Snapshot, NumOfSoftBytes);

      /* If one packet will do then send the packet */
      if (NumOfSoftBytes <= 8)
      {
         DataToSendWithoutTransport = CAN_cbTRUE;
         BuildPgn65242OnePacket (TxMsg);
      }
      else
      {
         ...
         if (! TxMsg->Requested)
         {
            ...
            if (! J1939_RequestTransportOutManagement (J1939_GLOBAL_DEST_ADDR,
                                                       NumOfSoftBytes, J1939_PGN_65242, ServicePGN65242TransportOut))
            {
...
/***************************************************************************/
/* FUNCTION: J1939_TxInit_PGN_65242                                        */
/* Description: Initialize hooks to can manager                            */
/***************************************************************************/
void J1939_TxInit_PGN_65242 (J1939_Transmit_Message_Control_T *MsgCtrl)
{
   MsgCtrl->Service_Routine = Service_PGN_65242;
   MsgCtrl->Time_To_Service       = 0x00;
   MsgCtrl->Tx_Timeout           = 0x00;
   MsgCtrl->Time_To_Next_Service_W = J1939_PGN_65242_FREQ;
   MsgCtrl->Tx_Timeout_Timer_W    = J1939_PGN_65242_TIMEOUT;
}
