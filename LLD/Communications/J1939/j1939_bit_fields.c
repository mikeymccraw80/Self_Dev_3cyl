 ...
 * Project: Bit field library                                                *
 * File Name:  j1939_bit_fields.c                                            *
 * File Description:                                                         *
 * This file contains bit field library function implementation              *
 *****************************************************************************/
#include "j1939_bit_fields.h"
#include "j1939_bit_fields_config.h"
/**************************************************************
 *  #defines
 **************************************************************/
...
const static uint8_t first_bit_mask_8[8] =
{
   0xfe, 0xfc, 0xf8, 0xf0,
   0xe0, 0xc0, 0x80, 0x00
};

const static uint8_t last_bit_mask_8[8]  =
{
   0x01, 0x03, 0x07, 0x0f,
   0x1f, 0x3f, 0x7f, 0xff
};

const static uint16_t first_bit_mask_16[16] =
{
...
const static uint16_t last_bit_mask_16[16] =
{
...
const static uint32_t first_bit_mask_32[32] =
{
...
const static uint32_t last_bit_mask_32[32] =
{
...
/* void Set_Long_Word_Bit(uint32_t *bit_fld_addr,
                       uint8_t   bit_no)
{
...
uint8_t  Read_Byte_Bit_Fld (const Byte_Bit_Fld_T *bit_fld)
{
...
/*                routine.                                            */
/**********************************************************************/
uint16_t  Read_Unsigned_Word_Bit_Fld (const Word_Bit_Fld_T *bit_fld)
{
   /* used to prevent compiler from using
    * hardware register as temporary storage
    */
...
uint16_t  Read_Unsigned_Word_From_Odd_Add_Bit_Fld (const Word_Bit_Fld_T *bit_fld)
{
   /* used to prevent compiler from using
    * hardware register as temporary storage
    */
   volatile uint16_t temp;
   uint8_t *ptr;
   ptr = (uint8_t *) bit_fld->addr;
#ifdef LITTLE_ENDIAN_MICRO
#ifdef INTEL_DBC
   temp = (uint16_t) (*ptr) | ( ( (uint16_t) (* (ptr + 1))) << 8); //No Shift
#else
   temp = ( ( (uint16_t) (*ptr)) << 8) | ( (uint16_t) (* (ptr + 1))); //Shift
#endif
#else
#ifdef INTEL_DBC
   temp = (uint16_t) (*ptr) | ( ( (uint16_t) (* (ptr + 1))) << 8); //No Shift
...
sint16_t  Read_Signed_Word_Bit_Fld (const Word_Bit_Fld_T *bit_fld)
{
   /* used to prevent compiler from using
    * hardware register as temporary storage
    */
   uint8_t *ptr;
   volatile sint16_t temp;
   ptr = (uint8_t *) bit_fld->addr;
   temp = ( (*ptr) << 8) | (* (ptr + 1));
...
uint32_t  Read_Unsigned_Long_Word_Bit_Fld (const Long_Word_Bit_Fld_T *bit_fld)
{
   /* used to prevent compiler from using
    * hardware register as temporary storage
    */
...
uint32_t  Read_Unsigned_Long_Word_From_Odd_Add_Bit_Fld (const Long_Word_Bit_Fld_T *bit_fld)
{
   /* used to prevent compiler from using
    * hardware register as temporary storage
    */
...
#ifdef LITTLE_ENDIAN_MICRO
#ifdef INTEL_DBC
   temp = ( (uint32_t) (*ptr)) | ( ( (uint32_t) (* (ptr + 1))) << 8) |
          ( ( (uint32_t) (* (ptr + 2))) << 16) | ( ( (uint32_t) (* (ptr + 3))) << 24); //No Shift
#else
   temp = ( ( (uint32_t) (*ptr)) << 24) | ( ( (uint32_t) (* (ptr + 1))) << 16) |
          ( ( (uint32_t) (* (ptr + 2))) << 8) | ( (uint32_t) (* (ptr + 3)));   //Shift
#endif
#else
#ifdef INTEL_DBC
   temp = ( (uint32_t) (*ptr)) | ( ( (uint32_t) (* (ptr + 1))) << 8) |
                          ...
                          uint8_t new_value)
{
   ...
   */
   temp = (*bit_fld->addr & mask) |
          ( (new_value & (last_bit_mask_8[bit_fld->first_bit - bit_fld->last_bit]))
            << bit_fld->last_bit);
   *bit_fld->addr  = temp;
}
                          ...
                          uint16_t new_value)
{
    ...
    */
   /* temp = ((uint16_t)(*bit_fld->addr) & (uint16_t) mask) |
             (new_value << bit_fld->last_bit);
   */
   temp = (temp & mask) |
...
#else
   *bit_fld->addr = ( (temp & 0x00ff) << 8) | ( (temp >> 8) & 0x00ff); //Shift
#endif
#else
#ifdef INTEL_DBC
...
#ifdef LITTLE_ENDIAN_MICRO
#ifdef INTEL_DBC
   *ptr     = (uint8_t) (temp & 0x00ff);
   * (ptr + 1) = (uint8_t) ( (temp & 0xff00) >> 8); //No Shift
#else
   *ptr     = (uint8_t) ( (temp & 0xff00) >> 8);
   * (ptr + 1) = (uint8_t) (temp & 0x00ff);    //Shift
#endif
#else
#ifdef INTEL_DBC
   *ptr     = (uint8_t) (temp & 0x00ff);
...
#ifdef LITTLE_ENDIAN_MICRO
#ifdef INTEL_DBC
   *ptr     = (uint8_t) (temp & 0x000000ff);
   * (ptr + 1) = (uint8_t) ( (temp & 0x0000ff00) >> 8);
   * (ptr + 2) = (uint8_t) ( (temp & 0x00ff0000) >> 16);
   * (ptr + 3) = (uint8_t) ( (temp & 0xff000000) >> 24); //No Shift
#else
   *ptr     = (uint8_t) ( (temp & 0xff000000) >> 24);
   * (ptr + 1) = (uint8_t) ( (temp & 0x00ff0000) >> 16);
   * (ptr + 2) = (uint8_t) ( (temp & 0x0000ff00) >> 8);
   * (ptr + 3) = (uint8_t) (temp & 0x000000ff);      //Shift
#endif
#else
#ifdef INTEL_DBC
   *ptr     = (uint8_t) (temp & 0x000000ff);
 ...
 * ===  ========  =============    ====================================        *
 * 1   14-May-09   Sandeep,      Created initial version of this file for      *
 *                 Prakash,      J1939 protocol                                *
 * 1   19-Oct-11   Arpit,         SOFC J1939 first release                     *
 *                 Anupindi,                                                   *
 ******************************************************************************/
