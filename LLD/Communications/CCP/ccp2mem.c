...
static CCP_CRC_T Write_To_Flash( uint8_t MTA_number, uint8_t data_to_write )
{
   ...
   else if( FLASH == CCP_Location_Of_Address_User( MTA_ptr[ MTA_number ] ) )
   {
...
static CCP_CRC_T Program_Data( uint8_t start_byte, uint8_t length )
{
   ...
   for ( loop_index = start_byte; loop_index < ( start_byte + length ); loop_index++ )
   {
      ...
      else if ( FLASH == CCP_Location_Of_Address_User( MTA_ptr[ MTA0_POINTER ] ) )
      {
...
CCP_CRC_T CCP_Build_Chksum( void )
{
   ...
   else
   {
       ...
       */
      ccp.Checksum         = 0;
      ccp.ChecksumSize     = READ_UNALIGNED_MEMORY_SIZE();
      ccp.delayed_response = true;
   }
   ...
   if ( ccp.ChecksumSize )
   {
      ...
      do
      {
         if ( (CPU_VERTICAL == CPU_Info)&&
              (0x40000000 == ((uint32_t)MTA_ptr[ MTA0_POINTER ]&0x40000000))
            )
         {
            // to maintain one A2L file, the start address of working page is defined as RAM2_START_ADDRESS
            // in A2L no matter vertical or lci is in use, this is convert lci working page address to 
            // vertical working page address
            MTA0_tmp_ptr = (uint8_t *)((uint32_t)EXTERNAL_RAM_START_ADDRESS +
                           (uint32_t)MTA_ptr[ MTA0_POINTER ] - (uint32_t)RAM2_START_ADDRESS);
         }
         else
         {
            MTA0_tmp_ptr = MTA_ptr[ MTA0_POINTER ];
         }

         /*temp_checksum += (uint16_t)(*MTA_ptr[ MTA0_POINTER ]);*/
         /*MTA_ptr[ MTA0_POINTER ]++;*/
         temp_checksum += (uint16_t)(*MTA0_tmp_ptr);
         MTA_ptr[ MTA0_POINTER ]++;
