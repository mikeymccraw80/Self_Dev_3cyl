 ...
 * for a specific DAQ List.
 */
static DAQ_List_Status_T DAQ_List_Status[ NUMBER_OF_DAQ_LISTS ];
                                  ...
                                  ODT_Element_T* DAQ_address_size )
{
   ...
   offset = (uint16_t)( ( CCP_DAQ_Configuration[ DAQ_number ].number_of_elements_per_ODT * ODT_number )
                         + element_number );
   ...
   size = (uint16_t)(*address & (uint16_t)(3U << remainder));
   size >>= remainder;

   if ( size < 2 )
   {
      DAQ_address_size->address = (HUGE uint8_t*)((((uint32_t)(*( CCP_DAQ_Configuration[ DAQ_number ].config_buffer_ptr + offset )))<< 1)
                                                                                                                 | (uint32_t)size );
      DAQ_address_size->size = ( 0 == DAQ_address_size->address  ) ? (uint8_t)NO_SIZE_XFER : (uint8_t)BYTE8_XFER;
   }
   else
   {
      DAQ_address_size->address = (HUGE uint8_t*)((uint32_t)(*( CCP_DAQ_Configuration[ DAQ_number ].config_buffer_ptr + offset ))<< 1);
                                             
      DAQ_address_size->size = ( 3 <= size ) ? (uint8_t)LONG32_XFER : (uint8_t)WORD16_XFER ;
   }
...
static void Write_to_Current_DAQ_Pointer( ODT_Element_T element )
{
   ...
   *( CCP_DAQ_Configuration[ DAQ_number ].config_buffer_ptr + offset ) = 
      (0 == element.size ) ? (uint16_t)0U : (uint16_t)(((uint32_t)element.address & 0x0001FFFFUL) >> 1);
   ...
   if ( element.size < 2 )
   {
       /*put the least significant bit of address into bit 0 of size */
      size =  (uint16_t)((uint32_t)element.address & 0x00000001UL);
   }
   else
   {
     size = ( element.size > 3 ) ? (uint16_t)3U : (uint16_t)element.size;
   }
...
#if ( FORCE_MSB_FIRST_DAQ_PROCESSING )
         /*
          * Consideration has to be taken here for endianness and coherency of data.
          * CCP expects data in BIG endian format. Copy 1, 2, or 4 bytes.
          * Addresses are aligned on their respective size boundary.
          */
         switch ( DAQ_address_size.size) //local_DAQ_element_size )
         {
            case BYTE8_XFER:
               destination_ptr[ message_byte_counter++ ] = DAQ_address_size.address;
               break;
            case WORD16_XFER:
               temp_value = *(HUGE uint16_t*)( DAQ_address_size.address );
               destination_ptr[ message_byte_counter++ ] = (temp_value & 0x0000FF00) >> 8;
               destination_ptr[ message_byte_counter++ ] = (temp_value & 0x000000FF);
               break;
            case LONG32_XFER:
               temp_value = *(HUGE uint32_t*)( DAQ_address_size.address );
               destination_ptr[ message_byte_counter++ ] = (temp_value & 0xFF000000) >> 24;
               destination_ptr[ message_byte_counter++ ] = (temp_value & 0x00FF0000) >> 16;
               destination_ptr[ message_byte_counter++ ] = (temp_value & 0x0000FF00) >> 8;
               destination_ptr[ message_byte_counter++ ] = (temp_value & 0x000000FF);
               break;
            default:
...
CCP_CRC_T CCP_Write_DAQ( void )
{
   ...
   temp_address         = FIX_ADDRESS( ccp.command_data.address.address_location32 );
   ...
   {
      new_ODT_element.address = (HUGE uint8_t *)temp_address;
