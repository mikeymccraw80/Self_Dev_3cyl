...
static bool                   SPARK_ISR_PulseRequested[SPARK_MAX_CYLINDERS];
...
void SPARK_Reset_Parameters( void )
{
    ...
    for (counter = 0; counter < SPARK_Number_Of_Cylinders; counter++ ) {
        SPARK_ISR_PulseRequested[counter] = false;
    }
...
// SPARK_Set_Angle
//=============================================================================
void SPARK_Set_Angle_In_Counts(Crank_Angle_T *in_spark_count_array)
{
    uint8_t  counter;
...
{
    uint32_t          cs;
    Crank_Cylinder_T  cylinder = 0, next_cylinder;
    uint32_t          counter;
    ...
    if( SPARK_Enable ) {
        if( SPARK_Get_Channel_Enable( SPARK_Cylinder_Event_ID ) ) {
            if(!SPARK_ISR_PulseRequested[SPARK_Cylinder_Event_ID]) {
                for( counter = 0; counter < SPARK_Control_Select_Max; counter++ ) {
                ...
                SPARK_ISR_PulseRequested[SPARK_Cylinder_Event_ID] = true;
            } else {
            ...
            SPARK_ISR_PulseRequested[SPARK_Cylinder_Event_ID] = false;
            next_cylinder = (SPARK_Cylinder_Event_ID + 1) < SPARK_Number_Of_Cylinders? (SPARK_Cylinder_Event_ID + 1) : 0;
            SPARK_ISR_PulseRequested[next_cylinder] = false;
        }
    }
