...
/******************************* CHANGES *********************************
 0.0.0   09.10.2008         Vikas Kothari	     Initial Version
 0.0.1   11.14.2008		    Chen He				 Correct bug for leaving
                                                 EHV high for MAS blks
...
*/
UINT32 ssd_c90fl_FlashErase ( PSSD_CONFIG pSSDConfig,
                    UINT8 shadowFlag,
                    ...
                    void (*CallBack)(void))
{
	...
	if(((lowEnabledBlocks | highEnabledBlocks) != 0) || ((shadowFlag & 0x01) == 0x01))
	{
		...
		returnCode |= CflashErase (&ssdConfig0, cShadowFlag, cLowEnabledBlocks, cMidEnabledBlocks, cHighEnabledBlocks);
	...
	if (((midEnabledBlocks | highEnabledBlocks) != 0) || ((shadowFlag & 0x02) == 0x02))
	{
		...
		/*invoke the common flash erase routine */
		returnCode |= CflashErase (&ssdConfig1, cShadowFlag, cLowEnabledBlocks, cMidEnabledBlocks, cHighEnabledBlocks);
		/*set flag to indicate bank zero is being erased*/
		bkFlag |= 0x02;
	}
	/* Check return value of the erase operation in this bank */
