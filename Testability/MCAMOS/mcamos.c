...
void MCAMOS_Process_7EE_Message(void)
{
   ...
   if(MCAMOS_enable)
   {
      if( Get_Message( MCAMOS_CANID_7EE, (uint8_t *)&MCAMOS_command) )
      {
...
{
   uint8_t transmit_message[8]={0,0,0,0,0,0,0,0};
   uint8_t idx;
   uint8_t bytecnt;
   ...
   if (MCAMOS_size > 0) 
   {
      for( idx = 0; idx < bytecnt; idx++)
      {
         transmit_message[idx] = *pmemloc;
         pmemloc++;
      }
      MCAMOS_size -= bytecnt;

      FlexCAN_Transmit_Message( MCAMOS_MESSAGE_TRANSMIT_CONFIGURATION, MCAMOS_CANID_7EF, transmit_message, 8);
   }
